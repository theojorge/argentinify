...
const Leaderboard: React.FC<LeaderboardProps> = ({ currentScore }) => {
  ...
  useEffect(() => {
    ...
    if (userInfo && userInfo.username) {
      setUsername(userInfo.username);
      console.log(
        `[Leaderboard] Username existente cargado desde userInfo: ${userInfo.username}`
      );
    }
  ...
  const submitRecommendation = async () => {
    ...
    console.log(
      `[Leaderboard] Enviando recomendación: ${artistRecommendation}`
    );
    ...
    try {
      ...
      if (response.ok) {
        ...
        setRecommendationMessage(
          `Error: ${errorData.error || "Error al enviar recomendación"}`
        );
      ...
      {userInfo &&
        ...
        !showUsernameInput && (
          <div style={styles.positionMessage}>
            Estás en la posición #{userInfo.position} de {userInfo.totalPlayers}{" "}
            jugadores
          </div>
        )}
