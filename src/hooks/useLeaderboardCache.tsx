...
export const useLeaderboardCache = () => {
    ...
    const loadLeaderboard = async (userId?: string) => {
        try {
            ...
            console.log(`[LeaderboardCache] Leaderboard cargado: ${data.leaderboard.length} entradas`);

            // Guardar en localStorage
            localStorage.setItem('cached-leaderboard', JSON.stringify(data.leaderboard));
    ...
    const updateUserInfo = (newUserInfo: UserInfo | null, newLeaderboard?: LeaderboardEntry[]) => {
        ...
        if (newLeaderboard) {
            ...
            localStorage.setItem('cached-leaderboard', JSON.stringify(newLeaderboard));
            localStorage.setItem('leaderboard-timestamp', Date.now().toString());
        }
    };
    ...
    useEffect(() => {
        ...
        if (cachedUserInfo) {
            try {
                ...
                console.error("[LeaderboardCache] Error cargando userInfo desde localStorage:", error);
            }
        }
    }, []);
